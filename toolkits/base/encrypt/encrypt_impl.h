...
{
public:
	bool  SetEncryptKey(const std::string &key, int key_len = 0) { return true; }
	bool  SetDecryptKey(const std::string &key, int key_len = 0) { return true; }
...
#if defined(OS_WIN)     
bool CreateGUID(std::string &guid)
{
	GUID  id;
...
public:
	...
	bool  SetEncryptKey(const std::string &key, int key_len)
	{
		key_ = key; 
		ExpandKey(key_len, key_);
		return true;
	}
	bool  SetDecryptKey( const std::string &key, int key_len)
	{
		key_ = key; 
		//int key_len = EVP_CIPHER_CTX_key_length();
		ExpandKey(key_len, key_);
		return true;
	}
	nbase::EncryptMethod Type() const { return T; }
	...
	bool  Encrypt(const void *sdata, size_t ssize, std::string &ddata)
	{
		...
		int chiper_blocksize = EVP_CIPHER_CTX_block_size(ctx);
		if (chiper_blocksize > 0)
			osize = (sizeof(out)/ chiper_blocksize) * chiper_blocksize;
	...
	bool  Decrypt(const void *sdata, size_t ssize, std::string &ddata)
	{
		...
		//    is there some bug?
		//    must ensure that the buffer size is count of block size
		int chiper_block_size = EVP_CIPHER_CTX_block_size(ctx);
		if (/*chiper_->block_size > 0*/chiper_block_size > 0)
			osize = (sizeof(out)/ chiper_block_size -1) * chiper_block_size;
		uint32_t block = (ssize+osize-1) / osize;
...
public:
	...
	{
		key_ = key; 
		key_len = EVP_CIPHER_CTX_key_length(ectx_);
...
private:
	const EVP_CIPHER           *chiper_;
	std::string                 key_;
	EVP_CIPHER_CTX              *ectx_ = NULL;            // encode (have state)
	bool                        ectx_valid_;      // if true , will EVP_CIPHER_CTX_cleanup
	EVP_CIPHER_CTX              *dctx_ = NULL;            // decode (have state)
...
public:
	...
	bool  SetEncryptKey(const std::string &key, int key_len)
	{
		if (!method_)
			return false;
		return method_->SetEncryptKey(key, key_len);
	}
	bool  SetDecryptKey(const std::string &key, int key_len)
	{
		if (!method_)
