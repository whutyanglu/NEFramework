...
{
	char* bits_len = new char[data_base_length_*4];
	char* hex_len = new char[data_base_length_*2];

	int iLen = 0;

	memset(szCiphertextData,0,data_base_length_);
	memset(hex_len,0,data_base_length_*2);
	

	iLen = (((int)strlen(szCipherInBytes)>>2) + ((int)strlen(szCipherInBytes) % 4 == 0 ? 0 : 1))<<4;
	
	memcpy(hex_len,szCipherInBytes,strlen(szCipherInBytes));
	Hex2Bits(hex_len,bits_len,iLen);
	Bits2Bytes(szCiphertextData,bits_len,iLen);
