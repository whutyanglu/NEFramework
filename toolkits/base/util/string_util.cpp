...
// 比较特殊
template<typename CharType>
void StringTrimT(std::vector<CharType> chs, std::basic_string<CharType> &output)
{
	if (output.empty())
		return;
	size_t bound1 = 0;
...
std::wstring & StringTrim(std::vector<wchar_t> chs, std::wstring & input)
{
	StringTrimT<wchar_t>(chs, input);
	return input;
}
