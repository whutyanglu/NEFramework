...
namespace ui
{
	...
	{
		pm_.RemovePreMessageFilter(this);
		pm_.ReapObjects(pm_.GetRoot());
	}
	...
	std::wstring WindowImplBase::QueryControlText(std::wstring lpstrId, std::wstring lpstrType)
	{
		return L"";
	}
	...
	BOOL WindowImplBase::IsInStaticControl(Control *pControl)
	{
		BOOL bRet = FALSE;
		if (!pControl){
			return TRUE;
		...
		std::vector<std::wstring> vctStaticName;
		strClassName = nbase::MakeLowerString(pControl->GetClass());

		vctStaticName.push_back(_T("control"));
		vctStaticName.push_back(_T("text"));
		vctStaticName.push_back(_T("label"));
		vctStaticName.push_back(_T("box"));
		vctStaticName.push_back(_T("hbox"));
		vctStaticName.push_back(_T("vbox"));
		vctStaticName.push_back(_T("tabbox"));
		vctStaticName.push_back(_T("childbox"));
		vctStaticName.push_back(_T("dialogbox"));
		vctStaticName.push_back(_T("progressbox"));

		std::vector<std::wstring>::iterator it = std::find(vctStaticName.begin(), vctStaticName.end(), strClassName);
		if (vctStaticName.end() != it){
			Control* pParent = pControl->GetParent();
			while (pParent){
				strClassName = nbase::MakeLowerString(pParent->GetClass());
	...
	LRESULT WindowImplBase::OnNcHitTest(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		RECT rcCaption = pm_.GetCaptionRect();
		...
		if (pt.x >= rcClient.left + rcCaption.left && pt.x < rcClient.right - rcCaption.right
			...
			Control* pControl = pm_.FindControl(pt);
	...
	LRESULT WindowImplBase::OnGetMinMaxInfo(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		::GetMonitorInfo(::MonitorFromWindow(wnd_, MONITOR_DEFAULTTOPRIMARY), &Monitor);
		...
		lpMMI->ptMinTrackSize.x = pm_.GetMinInfo().cx;
		lpMMI->ptMinTrackSize.y = pm_.GetMinInfo().cy;
	...
	LRESULT WindowImplBase::OnSysCommand(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		if( ::IsZoomed(*this) != bZoomed ) {
			if( !bZoomed ) {
				Control* pControl = static_cast<Control*>(pm_.FindControl(_T("maxbtn")));
				if( pControl ) pControl->SetVisible(false);
				pControl = static_cast<Control*>(pm_.FindControl(_T("restorebtn")));
	...
	LRESULT WindowImplBase::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		// 关联UI管理器
		pm_.Init(wnd_, GetManagerName());
		// 注册PreMessage回调
		pm_.AddPreMessageFilter(this);

		// 创建主窗口
		Control* pRoot=NULL;
		DialogBuilder builder;
		std::wstring sSkinType = GetSkinType();
		std::wstring skin_file = GetSkinFolder() + L"\\" + GetSkinFile();
		if (!sSkinType.empty()) {
			STRINGorID xml(_ttoi(skin_file.c_str()));
			pRoot = builder.Create(xml, sSkinType, &pm_);
		}
		else {
			pRoot = builder.Create(skin_file.c_str(), L"", &pm_);
		}

		if (pRoot == NULL) {
			std::wstring sError = _T("加载资源文件失败：");
			sError += GetSkinFile();
	...
	LRESULT WindowImplBase::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		if (pm_.MessageHandler(uMsg, wParam, lParam, lRes))
			return lRes;
