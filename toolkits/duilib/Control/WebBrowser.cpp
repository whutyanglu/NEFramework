...
namespace ui
{
	////////////////////////////////////////////////////////////////////////
	//
	IMPLEMENT_DUICONTROL(WebBrowser)
	...
	void WebBrowser::Navigate2(std::wstring s)
	{
		Navigate2(s.c_str());
	}
	...
	void WebBrowser::TitleChange(BSTR bstrTitle)
	{
		if (m_pWebBrowserEventHandler)
		{
	...
	void WebBrowser::SetWebBrowserEventHandler( CWebBrowserEventHandler* pEventHandler )
	{
	...
	void WebBrowser::NavigateUrl(std::wstring s)
	{
		NavigateUrl(s.c_str());
	}

	LPCTSTR WebBrowser::GetClass() const
	{
		return DUI_CTR_WEBBROWSER;
	}

	LPVOID WebBrowser::GetInterface( LPCTSTR pstrName )
	{
		if( _tcsicmp(pstrName, DUI_CTR_WEBBROWSER) == 0 ) return static_cast<WebBrowser*>(this);
