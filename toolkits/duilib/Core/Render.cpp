...
#	include "ximaenc.cpp"
#	include "ximagif.cpp"
#	include "ximainfo.cpp"
#	include "ximalpha.cpp"
#	include "ximapal.cpp"
#	include "ximatran.cpp"
#	include "ximawnd.cpp"
#	include "xmemfile.cpp"
#endif
...
namespace ui {
	...
	bool DrawImage(HDC hDC, PaintManager* pManager, const RECT& rc, const RECT& rcPaint, std::wstring sImageName, \
		std::wstring sImageResType, RECT rcItem, RECT rcBmpPart, RECT rcCorner, DWORD dwMask, BYTE bFade, \
		bool bHole, bool bTiledX, bool bTiledY, HINSTANCE instance = NULL)
	{
		...
		if( sImageResType.empty() ) {
			data = pManager->GetImageEx(sImageName, L"", dwMask, false, instance);
		}
		else {
		...
		RenderEngine::DrawImage(hDC, data->hBitmap, rcItem, rcPaint, rcBmpPart, rcCorner, pManager->IsLayered() ? true : data->bAlpha, bFade, bHole, bTiledX, bTiledY);
	...
	TImageInfo* RenderEngine::LoadImage(STRINGorID bitmap, std::wstring type, DWORD mask, HINSTANCE instance)
	{
		LPBYTE pData = NULL;
		DWORD dwSize = 0;
		do 
		{
			if( type.empty()) {
				...
				if( PaintManager::GetResourceZip().empty() ) {
					...
					std::wstring sFilePwd = PaintManager::GetResourceZipPwd();  //Garfield 20160325 带密码zip包解密
					HZIP hz = NULL;
				...
				{
					dllinstance = PaintManager::GetResourceDll();
				}
				HRSRC hResource = ::FindResource(dllinstance, bitmap.m_lpstr, type.c_str());
...
#ifdef USE_XIMAGE_EFFECT
	static DWORD LoadImage2Memory(const STRINGorID &bitmap, std::wstring type,LPBYTE &pData)
	{
		assert(pData == NULL);
		pData = NULL;
		DWORD dwSize(0U);
		do 
		{
			if( type.empty() )
			{
				...
				else 
				{
					sFile += PaintManager::GetResourceZip();
					HZIP hz = NULL;
					if( PaintManager::IsCachedResourceZip() ) 
						hz = (HZIP)PaintManager::GetResourceZipHandle();
					else {
			...
			{
				HINSTANCE hDll = PaintManager::GetResourceDll();
				HRSRC hResource = ::FindResource(hDll, bitmap.m_lpstr, type.c_str());
				if( hResource == NULL ) break;
	...
	CxImage* RenderEngine::LoadGifImageX(STRINGorID bitmap, std::wstring type , DWORD mask)
	{
		//write by wangji
		LPBYTE pData = NULL;
	...
	Gdiplus::Image* RenderEngine::GdiplusLoadImage(std::wstring pstrPath)
	{
		LPBYTE pData = NULL;
		DWORD dwSize = 0;
		...
		do 
		{
			...
			if( PaintManager::GetResourceZip().empty() ) {
				...
				ZIPENTRY ze; 
				int i; 
				if( FindZipItem(hz, pstrPath.c_str(), true, &i, &ze) != 0 ) break;
				dwSize = ze.unc_size;
				if( dwSize == 0 ) break;
				pData = new BYTE[ dwSize ];
				int res = UnzipItem(hz, i, pData, dwSize);
		...
		{
			//读不到图片, 则直接去读取bitmap.m_lpstr指向的路径
			HANDLE hFile = ::CreateFile(pstrPath.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
			if( hFile == INVALID_HANDLE_VALUE ) break;
			dwSize = ::GetFileSize(hFile, NULL);
			if( dwSize == 0 ) break;
	...
	Gdiplus::Image* RenderEngine::GdiplusLoadImage( LPVOID pBuf,size_t dwSize )
	{
	...
	void RenderEngine::FreeImage(TImageInfo* bitmap, bool bDelete)
	{
	...
	bool RenderEngine::DrawIconImageString(HDC hDC, PaintManager* pManager, const RECT& rc, const RECT& rcPaint, 
		std::wstring strImage, std::wstring pStrModify)
	{
		...
		std::wstring sImageName = strImage;
		...
		for( int i = 0; i < 2; ++i,image_count = 0 ) {
			...
			while( *pStrImage != _T('\0') ) {
				...
				if( !sValue.empty() ) 
				{
					...
					{
						if( image_count > 0 )
							ui::DrawImage(hDC, pManager, rc, rcPaint, sImageName, sImageResType,
		...
		ui::DrawImage(hDC, pManager, rc, rcPaint, sImageName, sImageResType, rcItem, rcBmpPart, rcCorner, dwMask, bFade, bHole, bTiledX, bTiledY);

		return true;
	}

	bool RenderEngine::MakeFitIconDest(const RECT& rcControl,const CDuiSize& szIcon, std::wstring sAlign, RECT& rcDest)
	{
		ASSERT(!sAlign.empty());
		if(sAlign == _T("left"))
		{
	...
	TImageInfo* RenderEngine::LoadImage(std::wstring pStrImage, std::wstring type, DWORD mask, HINSTANCE instance)
	{	
		if(pStrImage.empty()) return NULL;

		std::wstring sStrPath = pStrImage;
		if( type.empty())  {
			sStrPath = ResourceManager::GetInstance()->GetImagePath(pStrImage);
			if (sStrPath.empty()) sStrPath = pStrImage;
			else {
				/*if (ResourceManager::GetInstance()->GetScale() != 100) {
					std::wstring sScale;
					sScale.Format(_T("@%d."), ResourceManager::GetInstance()->GetScale());
	...
	TImageInfo* RenderEngine::LoadImage(UINT nID, std::wstring type, DWORD mask, HINSTANCE instance)
	{
		return LoadImage(STRINGorID(nID), type, mask, instance);
	}

	void RenderEngine::DrawText(HDC hDC, PaintManager* pManager, RECT& rc, std::wstring pstrText,DWORD dwTextColor, \
	...
	bool RenderEngine::DrawImageInfo(HDC hDC, PaintManager* pManager, const RECT& rcItem, const RECT& rcPaint, const TDrawInfo* pDrawInfo, HINSTANCE instance)
	{
		...
		return ui::DrawImage(hDC, pManager, rcItem, rcPaint, pDrawInfo->sImageName, pDrawInfo->sResType, rcDest,\
	...
	bool RenderEngine::DrawImageString(HDC hDC, PaintManager* pManager, const RECT& rcItem, const RECT& rcPaint,
		std::wstring pStrImage,  std::wstring  pStrModify, HINSTANCE instance)
	{
	...
	void RenderEngine::DrawRect(HDC hDC, const RECT& rc, int nSize, DWORD dwPenColor,int nStyle /*= PS_SOLID*/)
	{
#if USE_GDI_RENDER
		ASSERT(::GetObjectType(hDC) == OBJ_DC || ::GetObjectType(hDC) == OBJ_MEMDC);
	...
	void RenderEngine::DrawRoundRect(HDC hDC, const RECT& rc, int nSize, int width, int height, DWORD dwPenColor,int nStyle /*= PS_SOLID*/)
	{
		ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
	...
	void RenderEngine::DrawText(HDC hDC, PaintManager* pManager, RECT& rc, std::wstring pstrText, DWORD dwTextColor, int iFont, UINT uStyle)
	{
		ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
		if( pstrText.empty() || pManager == NULL ) return;
		...
		{
			HFONT hOldFont = (HFONT)::SelectObject(hDC, GlobalManager::GetFont(iFont));
			Gdiplus::Graphics graphics( hDC );
			...
			if ((uStyle & DT_CALCRECT) != 0)
			{
				...
				graphics.MeasureString(pstrText.c_str(), -1, &font, rectF, &stringFormat, &bounds);
			...
			{
				graphics.DrawString(pstrText.c_str(), -1, &font, rectF, &stringFormat, &brush);
			}
#else
		...
		else
		{
			...
			HFONT hOldFont = (HFONT)::SelectObject(hDC, GlobalManager::GetFont(iFont));
			::DrawText(hDC, pstrText.c_str(), -1, &rc, uStyle);
			::SelectObject(hDC, hOldFont);
		}
	}

	void RenderEngine::DrawHtmlText(HDC hDC, PaintManager* pManager, RECT& rc, 
		std::wstring strText, DWORD dwTextColor, RECT* prcLinks, std::wstring* sLinks, int& nLinkRects, UINT uStyle)
	{
		...
		ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
		if( strText.empty() || pManager == NULL ) return;
		...
		TEXTMETRIC* pTm = &GlobalManager::GetDefaultFontInfo(hDC)->tm_;
		HFONT hOldFont = (HFONT) ::SelectObject(hDC, GlobalManager::GetDefaultFontInfo(hDC)->handle_);
		::SetBkMode(hDC, TRANSPARENT);
		::SetTextColor(hDC, RGB(GetBValue(dwTextColor), GetGValue(dwTextColor), GetRValue(dwTextColor)));
		...
		if( ((uStyle & DT_CENTER) != 0 || (uStyle & DT_RIGHT) != 0 || (uStyle & DT_VCENTER) != 0 || (uStyle & DT_BOTTOM) != 0) && (uStyle & DT_CALCRECT) == 0 ) {
			...
			int nLinks = 0;
			DrawHtmlText(hDC, pManager, rcText, strText, dwTextColor, NULL, NULL, nLinks, uStyle | DT_CALCRECT);
			if( (uStyle & DT_SINGLELINE) != 0 ){
		...
		std::wstring pstrLineBegin = strText;
		auto pstrText = strText.c_str();
				...
				&& ( pstrText[2] == _T(' ') || pstrText[2] == _T('>') || pstrText[2] == _T('}') ) ) {
					...
					case _T('a'):  // Link
						{
							...
							if( iLinkIndex < nLinkRects && !bLineDraw ) {
								...
								while( *pstrText != _T('\0') && *pstrText != _T('>') && *pstrText != _T('}') ) {
									std::wstring pstrTemp = ::CharNext(pstrText);
							...
							if(clrColor == 0) GlobalManager::GetDefaultLinkFontColor();
							if( bHoverLink && iLinkIndex < nLinkRects ) {
								std::wstring *pStr = (std::wstring*)(sLinks + iLinkIndex);
								if( sHoverLink == *pStr ) clrColor = GlobalManager::GetDefaultLinkHoverFontColor();
							}
							...
							TFontInfo* pFontInfo = GlobalManager::GetDefaultFontInfo(hDC);
							if( aFontArray.GetSize() > 0 ) pFontInfo = (TFontInfo*)aFontArray.GetAt(aFontArray.GetSize() - 1);
							if( pFontInfo->underline_ == false ) {
								HFONT hFont = GlobalManager::GetFont(pFontInfo->name_.c_str(), pFontInfo->size_, pFontInfo->bold_, true, pFontInfo->italic_);
								if( hFont == NULL ) hFont = GlobalManager::AddFont(pFontInfo->name_, pFontInfo->size_, pFontInfo->bold_, true, pFontInfo->italic_);
								pFontInfo = GlobalManager::GetFontInfo(hFont, hDC);
								aFontArray.Add(pFontInfo);
								pTm = &pFontInfo->tm_;
								::SelectObject(hDC, pFontInfo->handle_);
					...
					case _T('b'):  // Bold
						{
							...
							TFontInfo* pFontInfo = GlobalManager::GetDefaultFontInfo(hDC);
							if( aFontArray.GetSize() > 0 ) pFontInfo = (TFontInfo*)aFontArray.GetAt(aFontArray.GetSize() - 1);
							if( pFontInfo->bold_ == false ) {
								HFONT hFont = GlobalManager::GetFont(pFontInfo->name_, pFontInfo->size_, true, pFontInfo->underline_, pFontInfo->italic_);
								if( hFont == NULL ) hFont = GlobalManager::AddFont(pFontInfo->name_, pFontInfo->size_, true, pFontInfo->underline_, pFontInfo->italic_);
								pFontInfo = GlobalManager::GetFontInfo(hFont, hDC);
								aFontArray.Add(pFontInfo);
								pTm = &pFontInfo->tm_;
								::SelectObject(hDC, pFontInfo->handle_);
					...
					case _T('f'):  // Font
						{
							...
							if( pstrTemp != pstrText ) {
								...
								sFontAttr = nbase::MakeLowerString(sFontAttr);
								if( sFontAttr.find(_T("bold")) != std::string::npos) bBold = true;
								if( sFontAttr.find(_T("underline")) != std::string::npos) bUnderline = true;
								if( sFontAttr.find(_T("italic")) != std::string::npos) bItalic = true;
								HFONT hFont = GlobalManager::GetFont(sFontName, iFontSize, bBold, bUnderline, bItalic);
								if( hFont == NULL ) hFont = GlobalManager::AddFont(sFontName, iFontSize, bBold, bUnderline, bItalic);
								TFontInfo* pFontInfo = GlobalManager::GetFontInfo(hFont, hDC);
								aFontArray.Add(pFontInfo);
								pTm = &pFontInfo->tm_;
								::SelectObject(hDC, pFontInfo->handle_);
							}
					...
					case _T('i'):  // Italic or Image
						{    
							...
							pstrText++;
							std::wstring sImageString = pstrText;
							int iWidth = 0;
							int iHeight = 0;
							while( *pstrText > _T('\0') && *pstrText <= _T(' ') ) pstrText = ::CharNext(pstrText);
							...
							if( sName.empty() ) { // Italic
								...
								TFontInfo* pFontInfo = GlobalManager::GetDefaultFontInfo(hDC);
								if( aFontArray.GetSize() > 0 ) pFontInfo = (TFontInfo*)aFontArray.GetAt(aFontArray.GetSize() - 1);
								if( pFontInfo->italic_ == false ) {
									HFONT hFont = GlobalManager::GetFont(pFontInfo->name_, pFontInfo->size_, pFontInfo->bold_, pFontInfo->underline_, true);
									if( hFont == NULL ) hFont = GlobalManager::AddFont(pFontInfo->name_, pFontInfo->size_, pFontInfo->bold_, pFontInfo->underline_, true);
									pFontInfo = GlobalManager::GetFontInfo(hFont, hDC);
									aFontArray.Add(pFontInfo);
									pTm = &pFontInfo->tm_;
									::SelectObject(hDC, pFontInfo->handle_);
					...
					case _T('u'):  // Underline text
						{
							...
							if( aFontArray.GetSize() > 0 ) pFontInfo = (TFontInfo*)aFontArray.GetAt(aFontArray.GetSize() - 1);
							if( pFontInfo->underline_ == false ) {
								HFONT hFont = GlobalManager::GetFont(pFontInfo->name_, pFontInfo->size_, pFontInfo->bold_, true, pFontInfo->italic_);
								if( hFont == NULL ) hFont = GlobalManager::AddFont(pFontInfo->name_, pFontInfo->size_, pFontInfo->bold_, true, pFontInfo->italic_);
								pFontInfo = GlobalManager::GetFontInfo(hFont, hDC);
								aFontArray.Add(pFontInfo);
								pTm = &pFontInfo->tm_;
				...
				case _T('u'):
					{
						...
						if( pFontInfo == NULL ) pFontInfo = GlobalManager::GetDefaultFontInfo(hDC);
						if( pTm->tmItalic && pFontInfo->italic_ == false ) {
							ABC abc;
	...
	SIZE RenderEngine::GetTextSize( HDC hDC, PaintManager* pManager , std::wstring pstrText, int iFont, UINT uStyle )
	{
		SIZE size = {0,0};
		ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
		if( pstrText.empty() || pManager == NULL ) return size;
		::SetBkMode(hDC, TRANSPARENT);
		HFONT hOldFont = (HFONT)::SelectObject(hDC, GlobalManager::GetFont(iFont));
	...
	void RenderEngine::CheckAlphaColor(DWORD& dwColor)
	{
	...
	HBITMAP RenderEngine::CreateARGB32Bitmap(HDC hDC, int cx, int cy, BYTE** pBits)
	{
		/*LPBITMAPINFO lpbiSrc = NULL;
		lpbiSrc = (LPBITMAPINFO) new BYTE[sizeof(BITMAPINFOHEADER)];
	...
	void RenderEngine::AdjustImage(bool bUseHSL, TImageInfo* imageInfo, short H, short S, short L)
	{
